...
def fetch_issue_statuses(
    ...
    Uses ``fetch_all_issues`` to exhaustively paginate all issues, then
    filters locally for the requested refs. This fixes the silent 1000-row
    cap that caused the 288-vs-713 NFMDP undercount.

    Returns a mapping of issue key → status string.
    Unresolved refs get status ``"unknown"``.
    """
    ...
    try:
        ...
        # Build a lookup from all issues keyed by issue key.
        issue_by_key: dict[str, str] = {}
        for issue in all_issues:
            key = issue.get("key")
            if key:
                issue_by_key[key] = issue.get("status", "unknown")

        for ref in issue_refs:
            statuses[ref] = issue_by_key.get(ref, "unknown")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Company UUID for the Paperclip issues API. "
            "Required for paginated issue lookups."
        ),
    )
    return parser
...
def main() -> None:
    ...
    statuses = fetch_issue_statuses(all_refs, args.api_url, args.company_id)
