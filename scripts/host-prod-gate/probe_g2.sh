...
for ENTRY in run-deploy.sh run-pre-deploy-assert.sh run-recovery.sh run-worker-inspect.sh run-sql.sh run-record-manifest.sh run-cleanup.sh run-backup.sh; do
