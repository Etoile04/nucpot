...
# NFM-4077: write the per-service env file consumed via `env_file:` in
# docker-compose.staging.yml.
#
# Why this exists
# ---------------
# docker-compose v5.5.0 YAML-re-parses interpolated ${VAR:-["..."]} values
# inside the compose file's `environment:` block as a YAML flow sequence and
# strips the inner double quotes on re-serialisation. The API container then
# receives ``NFM_CORS_ORIGINS=[http://...,https://...]`` (no inner quotes)
# and pydantic-settings' CORS_JSON parser crashes on startup with
# ``SettingsError: error parsing value for field "cors_origins"``.
#
# Writing the values into a plain env_file (KEY=VALUE, one per line) sidesteps
# YAML entirely — each value is forwarded to the container verbatim. The
# compose file's `env_file:` directive references this generated path.
#
# Generated path: docker/.env.staging.api (already matched by .gitignore's
# `docker/.env.*` rule — never committed).
write_api_env_file() {
  local api_env="$PROJECT_ROOT/docker/.env.staging.api"
  local database_url="${STAGING_DATABASE_URL:-}"
  local secret_key="${STAGING_API_SECRET_KEY:-}"
  # CRITICAL: read the CORS line directly from the env file rather than from
  # the shell var. ``set -a; . $ENV_FILE; set +a`` invokes bash's source
  # semantics which perform quote removal — the inner double quotes around
  # each origin get stripped, leaving ``[http://...,https://...]`` which
  # pydantic-settings' CORS_JSON parser refuses. Grepping the raw line lets
  # us forward the bracketed-JSON value verbatim.
  local cors_raw cors
  cors_raw="$(grep -E '^STAGING_CORS_ORIGINS=' "$ENV_FILE" 2>/dev/null || true)"
  if [ -z "$cors_raw" ]; then
    cors='["http://localhost:3010"]'
  else
    cors="${cors_raw#STAGING_CORS_ORIGINS=}"
  fi
  ...
  if [ -z "$secret_key" ]; then
    die "STAGING_API_SECRET_KEY is unset — set it in docker/.env.staging."
  fi

  cat > "$api_env" <<EOF
# Generated by scripts/staging_deploy.sh (NFM-4077).
# DO NOT EDIT BY HAND — regenerated on every deploy.
NFM_DATABASE_URL=${database_url}
NFM_SECRET_KEY=${secret_key}
...
cmd_deploy() {
  ...
  # NFM-4077: generate the per-service env file before any compose step reads
  # it (env_file: directive in docker-compose.staging.yml).
  write_api_env_file
