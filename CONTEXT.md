...
**每文献数据集 (per-literature dataset)**:
单篇文献的提取产物的私有容器。身份以 DOI 判重(内容哈希兜底);同一文献的重抽产生该容器的新版本而非新容器——「同文多源」结构性不可能(ADR-NFM-4535)。
_Avoid_: source 行(指语义容器时)、临时数据集

**校核门 (verification gate)**:
行进入可合并集合的前置条件:五动作校对通过 + 物理有效域通过;低置信度行必须人工通过。版本是校核通过行的快照。
_Avoid_: 审核流(泛指时)

**数据集版本 (dataset version)**:
合并集的快照(版本号/材料/行集引用/来源清单/时间)。回退 = 切换版本指针;支持整版本回退与按源回退两种语义。
_Avoid_: 数据快照(指版本时)、备份

**发布版 (published version)**:
供验证、物性页、检索与导出消费的数据集版本;工作版供校对页。AutoVC 验证结果记录所用版本号,等级可追溯。
_Avoid_: 正式数据(口语)

**按源回退 (per-source rollback)**:
从当前版本移除某篇文献的全部贡献后形成新版本的操作,带审计记录。「摄取层一次意外毁掉整个数据集」的兜底(ADR-NFM-4535)。
_Avoid_: 删数据(无版本语义时)

**物理有效域 (physical validity range)**:
属性级的取值合理区间(材料数据 schema 承载)。越界的提取结果不进入可合并集,标「物理无效·需核实」并给出原因。
_Avoid_: 格式校验(指类型/格式时)
