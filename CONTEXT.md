...
**平台背景(工作流总览)**:势函数的验证需要实验测量数据集支撑——验证状态、验证等级与质量等级衡量的都是「基于特定势函数的分子动力学计算结果」与「验证数据集」的偏差。数据集从文献库(本地 Zotero)与 Materials Project 等外部源获取;材料知识图谱由抽取管线对文献库 PDF 建立材料↔性能关联,帮助 LLM 理解成分/工艺/性能的映射、避免跨材料混淆;每条数据带来源标注,低置信度提取结果经人工校对(操作页并排展示提取结果与原文相关段落);材料数据 schema 描述数据集应含内容并自动评估完整性;缺失项在数据表做缺口标识,由智能体数据挖掘工作流按关键词从互联网搜索、下载文献入库后走既有抽取流程——闭环反哺验证。数据录入支持两条通道——网页页面人工录入与智能体驱动的自动提取——共用同一套数据后端;自动提取由专用材料提取技能(nuclear-materials-skills)执行。
...
**录入通道 (entry channel)**:
数据进入平台的两种方式:网页录入(人工在操作页填写)与智能体提取(自动抽取流水线)。两通道写入同一套数据后端、遵守同一来源标注与人工校对规则——通道不同,数据语义与质量门槛不降。
_Avoid_: 手工导入(指网页录入时)、自动导入(指智能体提取时)

### Wayfinder 同步

数据供给闭环的完整设计在 wayfinder 地图 [NFM-3830](/NFM/issues/NFM-3830) (PILOT-C G3 数据采集闭环修复设计地图) 的 Notes / 决策注释中沉淀;新增的七个术语让那张地图上的 G3 闭环拥有领域语言入口。后续 wayfinder 地图(数据挖掘 / 缺口闭环相关)开 PR 时,Notes 应锚定本节术语而非另起本地词汇。
