...
| 全天 | **混合焓查表数据准备**：Miedema模型参数收集（U-X二元对混合焓），整理为Python查表结构 | ML工程师/CTO | `mixing_enthalpy_data.py` | 1.0d |
...
| 时段 | 任务 | 负责角色 | 产出 | 工时 |
|------|------|---------|------|------|
| 上午 | **ClusterCompositionGenerator骨架**：`classify_cluster_type()` + `generate_candidates()` 骨架代码 | ML工程师 | `cluster_model.py` 骨架 | 0.5d |
| 上午 | DFT数据模型Alembic迁移+ORM模型 | 后端工程师 | Alembic migration + model | 0.5d |
| 下午 | **特征工程管道骨架**：Mo当量/电负性差/构型熵计算函数 | ML工程师 | `feature_engineering.py` 骨架 | 0.5d |
...
| 时段 | 任务 | 负责角色 | 产出 | 工时 |
|------|------|---------|------|------|
| 07-19 | **团簇模型完整实现**：四类构型分类 + 候选成分生成（5000样本） + 输出校验 | ML工程师 | 完整`cluster_model.py` | 1.0d |
| 07-19 | 特征计算剩余函数：B/V比/铀密度/混合焓/晶格畸变 | ML工程师 | 8个特征函数完成 | 0.5d |
...
| 07-20 | **ML分类模型初步训练**：用55组实验数据快速训练RandomForest基线，验证pipeline跑通 | ML工程师 | PhaseClassifier v0.1 + accuracy report | 0.5d |
...
| 上午 | FeatureEng管道：与团簇模型输出对接，批量特征计算pipeline | ML工程师 | Pipeline可运行 | 0.5d |
| 下午 | ML回归模型初步训练：GPR基线，验证pipeline跑通 | ML工程师 | TempPredictor v0.1 + MAE report | 0.5d |
| 下午 | 历史Materials数据特征回填脚本 | ML工程师/后端 | 回填脚本 | 0.5d |
...
| 全天 | 批量特征回填执行（异步） | ML工程师 | 历史数据特征值写入DB | 0.5d |
...
| 时段 | 任务 | 负责角色 | 产出 | 工时 |
|------|------|---------|------|------|
| 全天 | **ML分类模型正式训练**：RF + XGBoost ensemble，5-fold CV，>75%目标 | ML工程师 | PhaseClassifier v1.0 | 1.5d |
| 全天 | **ML回归模型正式训练**：GPR + SVR ensemble，LOO-CV，<40℃目标 | ML工程师 | TempPredictor v1.0 | 1.5d |
...
| 时段 | 任务 | 负责角色 | 产出 | 工时 |
|------|------|---------|------|------|
| 全天 | **训练数据增强**：整合1200组DFT计算数据到训练集（需院内DFT专家配合导出数据） | ML工程师+DFT专家 | 扩展训练集 | 1.0d |
| 全天 | 特征重要性分析：SHAP values，输出特征重要性排序 | ML工程师 | Feature importance report | 0.5d |
| 全天 | EnergyPredictor（结合能回归）初步训练 | ML工程师 | EnergyPredictor v0.1 | 0.5d |
...
| 全天 | 模型版本管理：ML模型序列化+版本号+v1.0.0 tag | ML工程师 | 模型版本化方案 | 0.5d |
| 全天 | Sprint 4 DoD逐项验收 | CTO | 验收报告 | 0.5d |
| 全天 | Sprint 5 准备：NSGA-II原型验证（小规模测试） | ML工程师 | pymoo集成POC | 0.5d |
...
| 角色 | Batch 1 (d) | Batch 2 (d) | 总计 (d) | 备注 |
|------|------------|------------|---------|------|
| ML工程师 | 4.0 | 5.0 | **9.0** | 核心资源，不可替代 |
| 后端工程师 | 2.0 | 2.5 | **4.5** | 可部分并行 |
| CTO | 1.0 | 0.5 | **1.5** | 技术方案+验收 |
| DFT/材料专家 | 0.5 | 1.0 | **1.5** | 院内配合，被动参与 |
| **合计** | **7.5** | **9.0** | **16.5** | — |

> 💡 Sprint 4 总工时16.5人天，集中在ML工程师（9天）。ML工程师是**唯一瓶颈资源**。
...
| 时段 | 任务 | 负责角色 | 产出 | 工时 |
|------|------|---------|------|------|
| 全天 | **NSGA-II核心集成**：pymoo Problem定义、目标函数（铀密度/相稳定温度/可制备性）、约束条件 | ML工程师 | NSGA-II core module | 1.5d |
...
| 时段 | 任务 | 负责角色 | 产出 | 工时 |
|------|------|---------|------|------|
| 全天 | **NSGA-II+ML代理模型集成**：用ML预测替代昂贵的DFT计算，加速优化评估 | ML工程师 | ML surrogate integration | 1.0d |
...
| 全天 | 收敛曲线输出：NSGA-II generational distance + hypervolume指标 | ML工程师 | Convergence metrics | 0.5d |
| 全天 | **DFT增量数据整合**：200组新DFT数据导入+特征计算+训练集扩展 | ML工程师+DFT专家 | 扩展训练集 | 1.0d |

#### Day 7-8（08-06 Thu — 08-07 Fri）

| 时段 | 任务 | 负责角色 | 产出 | 工时 |
|------|------|---------|------|------|
| 全天 | **ML模型再训练（v1.1）**：扩展训练集，目标分类>78%、MAE<35℃ | ML工程师 | ML v1.1 models | 1.0d |
...
| 任务 | 主负责 | 协助 | 院内配合 | 估计工时 |
|------|--------|------|---------|---------|
| 混合焓查表准备 | ML工程师 | CTO | DFT/材料专家 | 1.0d |
| ClusterCompositionGenerator | ML工程师 | — | — | 1.5d |
| 特征工程管道 | ML工程师 | 后端工程师 | — | 1.5d |
| ML分类模型训练 | ML工程师 | — | — | 2.5d |
| ML回归模型训练 | ML工程师 | — | — | 2.0d |
| DFT数据模型+API | 后端工程师 | — | DFT/材料专家 | 1.0d |
| 批量特征回填 | ML工程师 | 后端工程师 | — | 1.0d |
| ML预测API | 后端工程师 | ML工程师 | — | 1.0d |
| NSGA-II集成 | ML工程师 | — | — | 2.5d |
| 设计工作台UI | 前端工程师 | — | — | 3.0d |
| 势函数验证联动 | 全栈工程师 | — | — | 1.0d |
...
| 情景 | 影响 | 缓解方案 |
|------|------|---------|
| **ML工程师07-20前不到位** | Sprint 4 Batch 1无法启动ML训练 | CTO临时接管ML任务（CTO有ML背景），降低精度目标到>70% |
| **ML工程师07-22前不到位** | 初赛材料缺少ML部分 | Plan B-2：聚焦团簇模型+特征工程（纯物理方法），ML部分标注"开发中" |
| **ML工程师完全不到位** | Sprint 4-5 ML任务全部受阻 | CEO紧急协调院内资源；外包备选（Upwork ML freelancer，3天onboard） |

> ⚠️ **当前状态**：ML工程师标注为"🔄 待确认"。这是Sprint 4-5的**最高风险项**。建议CEO在07-19前确认到位状态。
...
| 基于路线图版本 | v1.6 |
| 状态 | Draft — 待CEO确认 |
| 下一步 | CEO确认后，由CPO拆分为Paperclip子任务并分配执行 |
