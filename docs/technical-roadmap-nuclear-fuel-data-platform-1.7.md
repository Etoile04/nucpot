...
> **版本说明：** v1.7 增补合同履约维度，对齐中国核动力研究设计院发布的《核级材料关系型数据库规范与开发技术规格书》(FMDB-410400-GG1,版本A,2026-07-29) 硬约束：① §2.2 已建系统能力全景增补 "数据汇交 1+N 架构" 评估（合同 M2 模块,当前 0% 未建）；② §2.3 能力成熟度评估新增"合同规格书 14 项硬指标"分项；③ §3.1 五层架构增补 "汇交中心节点 + 离线同步队列"；④ §3.2 技术选型增补 Neo4j→Apache AGE 替换说明 + DNA 标识/密级标签/断点续传三项入库；⑤ §4.3 分级分类与安全增强升格为"合同级强制" + DNA 水印 + 6 个月审计日志；⑥ 新增 §7.9 合同履约轨道 C,严格对齐 M1-M6 节点 + D0+18 验收时间线；⑦ 新增 §12 风险表 R11-R14 四项合同履约风险。
>
> **配套附件**：[合同规格书 md 整理版（FMDB-410400-GG1-specification.md）](contracts/FMDB-410400-GG1-specification.md) — 原 .doc 文件结构化后的 Markdown 版本，保留原文章节号(§3.1.2 等)以便交叉引用。
>
> v1.6 补充文献存储与合规技术栈调研成果:§3.2技术选型新增文件存储层（StorageBackend抽象+MinIO→信创演进），§7.8新增基础设施与合规并行轨道（涉密+信创三阶段路径+国密改造+测评周期），§4.3安全增强新增涉密信创维度。v1.5 CTO完成§3.2双轨数据流动与模块共享治理全部技术细节编写（R17 CTO补充）：数据写入统一接口Service Layer Gate（三层校验架构+血缘注入+质量评分路由+事件发布）、数据血缘跨产品线追踪（DataProvenance/DataLineageEdge模型+溯源查询API+Agent导出支持）、共享层模块治理模型（六层治理等级+RFC模板+Sprint双线验证检查清单）、Schema版本管理协议（7类变更策略+Pydantic版本化+API路由策略+兼容性测试自动化）、读取一致性与回传闭环设计。v1.4整合CEO第二轮反馈（R16-R17）：新增§1.7实验数据收集录入模块设计、§3.2双轨数据流动与模块共享治理（CTO编写占位）、升级§2.3能力成熟度新增实验数据录入行、升级§3.1五层架构图Web应用层新增实验数据工作台、升级§7.4 Sprint 6实验数据录入表单为完整模块。v1.3在v1.2基础上，新增CEO双产品线战略愿景（§1.6），将平台定位从单一Web产品升级为"Web交互产品 + 智能体开发平台"双轨架构。
...
7. [分阶段实施路线图](#7-分阶段实施路线图)
   - §7.9 v1.7 新增:轨道 C 合同履约(M1-M6 + D0+18 验收时间线 + M2 数据汇交专项)
...
> **合同硬约束**：《核级材料关系型数据库规范与开发技术规格书》(FMDB-410400-GG1) §2.2 明确要求 1 中心 + N 资源节点架构,DNA 标识/密级标签强制绑定,断点续传,汇交日志。本节按合同验收门槛评估当前实现。

| 能力模块 | 实现状态 | 合同要求 | 差距 |
|----------|---------|---------|------|
| **中心节点 (Hub)** | ❌ 完全缺失 | 1 中心,统一收口 | 需新建 `hub_nodes` 表 + Hub UI + 中心管理 API |
| **资源节点 (Resource)** | ❌ 完全缺失 | N 个资源节点(初始 ≥5) | 需新建 `resource_nodes` 表 + 节点注册/心跳/发现 |
| **DNA 标识** | ❌ 完全缺失 | 每条数据强制绑定 data_dna (UUID + SHA-256/SM3) | 需在 `data_sources` / `property_measurements` / `materials` 增列 |
| **密级标签** | ⚠️ 部分 | 非密/内部/秘密 三级分类,与 DNA 绑定 | 现有 `classification_level` 字段需扩展 + 强制非空 |
| **断点续传** | ❌ 完全缺失 | 大文件分块 + checkpoint + resume_token | 需新建 `upload_sessions` 表 + 分块上传 API |
| **汇交日志** | ❌ 完全缺失 | who/what/when/from_node/result 完整留痕 ≥6 月 | 需新建 `ingest_logs` 表 + 留存策略 |
| **离线操作** | ❌ 完全缺失 | 资源节点离线 100% 任务成功率 | 需离线缓存队列 + 重连冲突合并策略 |
| **跨节点同步** | ❌ 完全缺失 | 重连后 100% 同步成功率 | 需双向同步引擎 + 冲突解决 (vector clock / last-write-wins) |
| **多模态 V4 提取** | ✅ 完整闭环 | 4+ 文档格式 | ✅ 满足 (PDF/表格/图像 已实装) |

#### 综合管控层（合同 M5 模块, 部分实现）

| 能力模块 | 实现状态 | 合同要求 | 差距 |
|----------|---------|---------|------|
| **3 级组织架构** | ⚠️ 部分 | 院/所/课题组 三级 | 需新建 `organizations` 表 + parent_org_id 递归 + 行级权限 |
| **权限分级** | 🟡 基础 | 字段级/行级/操作级 | RBAC 已有,需扩展 RLS 策略 |
| **审计日志 (6月)** | 🔴 缺失 | 全量操作日志,留存 ≥6 个月 | 需新建 `audit_logs` 表 (who/what/when/where/result) + cron 归档 |
| **操作日志** | 🟡 基础 | 关键操作全留痕 | `hpc_failover_event` 是雏形,需扩到所有写操作 |
| **数据来源记录** | ✅ 满足 | 100% 抽查 10 条全可溯源 | ✅ `data_sources.file_path` + `parse_status` + `extraction_results.source_paragraph/page` 完整 |
...
> 对照《核级材料关系型数据库规范与开发技术规格书》§3 验收门槛 14 项硬指标的当前实现状态。**3 项已满足 / 6 项部分满足 / 5 项缺失**,其中 3 项为"100%"类硬指标(批量导入/同步/文档),需重点关注。
...
| 6 | 关系抽取准确率 | ≥80% | 关系抽取 pipeline 已建;无 CNAS 量化 | 🔴 | 同上 |
| 7 | 数据消歧准确率 | ≥90% | `entity_linker.py` 4 级消歧已建;无量化 | 🟡 | 需自评指标 + 内部测试集 |
...
**小结**：14 项指标中 **5 项🔴**(实体/关系/同步/压测/培训) 与 **M2 数据汇交模块**强耦合,必须 M3 节点(D0+5)前完成 M2 架构基础。
...
| **数据 DNA 标识**（合同硬要求）| UUID v4 + SHA-256/SM3 双 hash,每条核心数据必填 | 合同 M2 强制约束,需在 `data_sources`/`property_measurements`/`materials` 增列 |
| **密级标签**（合同硬要求）| 枚举: 非密/内部/秘密,DNA 强绑定 | 合同 M2 + M5 强制约束,继承现有 `classification_level` 字段 + 强制非空 + 与 DNA 联动校验 |
| **断点续传**（合同硬要求）| 分块上传 + resume_token + sha256 分块校验 | 合同 M2 强制约束,需新建 `upload_sessions` 表 + 分块 API |
...
### 4.3 分级分类与安全增强（v1.7 升格为"合同级强制"）

> **v1.7 升格说明**：v1.6 中分级分类为"数据治理增强方案";v1.7 根据合同规格书 §2.2 (M2 数据汇交) + §3 (M5 综合管控) 升格为"**合同级强制**",部分 P1 项必须 M3 节点前完成,不再可降级。

| 安全增强项 | 实现方案 | 优先级 | 合同约束 | 验收节点 |
|-----------|---------|--------|---------|----------|
| **数据 DNA 标识**（合同硬要求）| 新增 `data_dna` 列 (UUID v4 + SHA-256/SM3 双 hash),`data_sources` / `property_measurements` / `materials` 强制非空 | 🔴 **P0** | M2 | **M3 节点(D0+5)** |
| **密级标签**（合同硬要求）| 扩展 `classification_level` 字段,枚举 非密/内部/秘密,与 DNA 联动校验 | 🔴 **P0** | M2 + M5 | **M3 节点(D0+5)** |
| **断点续传**（合同硬要求）| 新建 `upload_sessions` 表 + 分块上传 API + resume_token + sha256 分块校验 | 🔴 **P0** | M2 | M4 节点(D0+10) |
| **汇交日志 ≥6 月**（合同硬要求）| 新建 `ingest_logs` 表 (who/what/when/from_node/result) + cron 归档策略 | 🔴 **P0** | M2 + M5 | M3 节点(D0+5) |
| **审计日志 ≥6 月**（合同硬要求）| 新建 `audit_logs` 表 (who/what/when/where/result) + 全 API 自动写日志 + 6 月 cron 归档 | 🔴 **P0** | M5 | M4 节点(D0+10) |
| **数据分级标签** | 继承 `classification_level` 字段 (公开/受限公开/秘密) | 🟡 P1 | M5 | M4 节点 |
| **版本快照** | 为核心数据表增加版本快照机制，支持回溯 | 🟡 P1 | M5 | M5 节点 |
| **数据 DNA 水印** | 导出 CSV/JSON 时嵌入不可见特征码（基于 data_dna 派生），关联用户账号 | 🟡 P1 | M2 | M4 节点 |
| **数据血缘可视化** | 前端展示数据加工链路图 | 🟢 P3 | — | Sprint 8 |
| **hash 算法参数化** | sha256/SM3 可通过环境变量切换，为涉密国密改造预留（SM3 输出长度与 SHA-256 一致，列定义不变） | 🟡 P1 | 合同 §4.1 | M3 节点 |
| **加密算法抽象** | 应用层加密接口预留 SM4，不绑定 AES | 🟢 P2 | 合同 §4.1 | M4 节点 |
| **LLM 调用本地化** | 确保 Ollama 可独立运行，涉密网络不能调外部 API | 🟡 P1 | 合同 §4.2 | M3 节点 |
| **网络入口双模式** | 预留内网 nginx 反代模式，涉密时删除 CF Tunnel | 🟢 P2 | 合同 §4.2 | M4 节点 |

> **合同 M5 综合管控 100% 验收门**：M6 节点(D0+18)前必须完成 P0 项全部 5 项(数据 DNA + 密级标签 + 断点续传 + 汇交日志 + 审计日志),任何一项缺失将影响合同验收。
...
### 7.9 轨道 C: 合同履约（v1.7 新增, 与轨道 A/B 并行）

> **v1.7 增补说明**：v1.6 仅含赛道 A(功能开发) + 轨道 B(基础设施与合规)。v1.7 根据合同规格书新增**轨道 C: 合同履约**,严格对齐 M1-M6 节点 + D0+18 验收时间线。**轨道 C 是甲方验收硬约束,优先级 ≥ 轨道 A,执行不可降级**。

#### 7.9.1 合同节点与时间线

| 节点 | 时间 | 交付物 | 负责人 | 验收方 |
|------|------|--------|--------|--------|
| **M1** | D0+1 | 《项目开发方案》(规格书解读 + 团队分工 + 风险预案) | CPO | 甲方 |
| **M2** | D0+2 | 系统架构设计文档(含 M2 数据汇交 1+N 架构详细设计) | CTO | 甲方 |
| **M3** | D0+5 | 核心模块（混合存储 + **汇交** + 管控）功能演示 | LE | 甲方 |
| **M4** | D0+10 | 知识图谱 + 机器学习 + **第三方 CNAS 测试报告** | E2E QA + CTO | 甲方 + CNAS |
| **M5** | D0+16 | 压力测试报告 + 部署运维报告 | SRE | 甲方 |
| **M6** | D0+18 | **项目验收**(甲方指定场地) | CPO + CEO | 甲方 |

> ⚠️ **关键风险**：`M3` 节点(混合存储 + **汇交** + 管控) 中的"**汇交**"模块当前 0%,按 Sprint 7-8 节奏不可达。**必须立即启动 M2 数据汇交 8-10 周专项 Sprint,挂入轨道 C**。

#### 7.9.2 M2 数据汇交专项 Sprint(8-10 周, 优先启动)

**目标**:完成合同 M2 全部 5 项核心能力(中心节点 / 资源节点 / DNA / 密级 / 续传),满足 M3 节点演示。
...
| W4 | DNA 标识生成服务(集成到所有写路径) | Service Layer Gate 自动注入 data_dna | BE |
| W4 | 密级标签强制校验(写前/写后双重校验) | `classification_level` Pydantic 校验 + DB CHECK 约束 | BE |
| W5-6 | 资源节点离线缓存队列(SQLite 本地存储 + 重连同步) | `nfm_node_client.upload_queue` | BE |
| W6-7 | 跨节点同步引擎(vector clock + last-write-wins + 人工合并) | `nfm_node_client.sync_engine` | BE |
| W7-8 | E2E 集成测试(5 节点模拟 + 网络分区场景) | `tests/e2e/test_hub_resource_sync.py` | E2E QA |
| W8-10 | 资源节点管理 UI + 离线/在线状态可视化 | `/admin/hub/nodes` 页面 | FE |
| W10 | M3 节点演示脚本 + 录屏 | `demos/M3-hub-resource-flow.md` + `.mp4` | CPO |

#### 7.9.3 M4 节点(知识图谱 + ML + CNAS 测试)前置

| 项 | 启动时间 | 周期 | 备注 |
|----|---------|------|------|
| 内部预测试集构建(实体/关系/消歧) | 立即 | 4 周 | 1000+ 条标注数据 |
| CNAS 测试机构接洽 | M2 启动后 2 周 | 2 个月排期 | 至少 3 家备选 |
| 内部准确率自评报告 | M2 启动后 6 周 | 2 周 | 实体 ≥85% / 关系 ≥80% / 消歧 ≥90% |
| CNAS 正式测试 | D0+8 | 2 周 | 准备期 + 测试期 |

#### 7.9.4 M5 节点(压力测试 + 部署运维)

| 测试项 | 工具 | 阈值 |
|--------|------|------|
| 批量导入压测 | Locust | 100% 成功率 @ 1000 req/s |
| KG 查询压测 | k6 | ≤5s @ 1k 节点/10k 关系 |
| 跨引擎复杂查询 | k6 | ≤5s @ 联合查询 |
| 同步引擎压测 | Locust | 100% 同步成功率 @ 网络分区恢复 |
| 部署运维剧本 | Ansible + Runbook | 6 个核心服务一键部署/回滚 |

#### 7.9.5 合同 IP 边界声明

> **IP 归属**：乙方(本项目)定制开发成果归甲方所有;乙方原有产品(nucpot 平台)仍归乙方所有。
> **培训要求**：≥12 学时,课堂讲述 + 上机操作。
> **保密要求**:"本文知识产权属于中国核动力院,未经我院书面同意,不得复制、传播、发表和用于其他方面"。
> **维保期**：验收完成起 1 年。

#### 7.9.6 轨道 C 与轨道 A/B 的关系

```
轨道 A(功能开发)          轨道 B(基础设施)         轨道 C(合同履约) [v1.7 新增]
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────────────┐
│ Sprint 4-8      │     │ B-Phase 1-4B    │     │ M1 → M2 → M3 → M4       │
│ + 智能设计引擎  │     │ 存储+合规       │     │   → M5 → M6 (D0+18)    │
│ + 实验数据工作台│     │ + 信创/涉密     │     │ + M2 数据汇交专项 8 周  │
└─────────────────┘     └─────────────────┘     └─────────────────────────┘
        │                       │                          │
        └───────────────────────┴──────────────────────────┘
                                │
                v1.7 商业策略补充: 合同履约优先, GTM 推迟到 D0+18 后
```

> **决策建议**:
> - **P0** (立即启动): 轨道 C M2 数据汇交专项 Sprint + 内部预测试集构建
> - **P1** (Sprint 5-6 并行): 轨道 A 智能设计核心 + 轨道 B B-Phase 1 存储前置
> - **P2** (D0+18 验收后启动): GTM 商业化(原 v1.6 战略,推迟到合同验收完成)
