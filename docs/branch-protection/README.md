...
> **Recovery note (NFM-3876).** The original spec lived on
> `origin/NFM-3861-branch-protection` (commit
> `ccc1881f5d37d11a4fe28f3d2247405a1944a881`). That slug matched the
> NFM-2770 branch-hygiene sweep pattern, putting the ref at risk of
> pruning. This directory is the durable re-shipment on
> `origin/sre/branch-protection-spec-v2` and contains identical
> content. See NFM-3876 for the recovery trail.
...
Hotfix branches carry the full set of `main`-class constraints
(linear history + force-push ban + required checks), with
`allow_deletions: false` so the hotfix branch remains as an audit
trail even after the squash-merge lands on `main`.
...
- **Recovery issue:** [NFM-3876](https://paperclip/NFM/issues/NFM-3876) (Phantom-pass recovery for NFM-3861)
