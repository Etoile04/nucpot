...
All extraction call-sites use `trigger_extraction_pipeline(source_reference, source_type, **kwargs)` from `apps.api.src.nfm_db.services.extraction_pipeline_dispatch`. The wrapper is the **only** external entry point that consults `EXTRACTION_PIPELINE_V2` / `NFM_EXTRACTION_V2_ENABLED`. The inline flag routing in `trigger_extraction()` (the legacy path body) is untouched — it remains the default-OFF fallback that the wrapper delegates to.
...
- No flag-flip in production as part of this ADR. The flag stays OFF. D3 + a code-complete B2/B3 chain are prerequisites.
...
- [ ] **D1:** Production deploy relies on the V2 pipeline by default — **PENDING**. The flag-flip is implemented on branch `origin/NFM-2876-flip-extraction-v2-default` (PR [#790](https://github.com/Etoile04/nucpot/pull/790) / commit `573ddc48`) but it is **not yet an ancestor of `main`**. The default on `main` is `False` (source of truth: [`apps/api/src/nfm_db/config.py:51`](../../apps/api/src/nfm_db/config.py)); V2 is opt-in today via `NFM_EXTRACTION_V2_ENABLED=true`. When the flip merges, `NFM_EXTRACTION_V2_ENABLED=false` becomes the documented rollback; this AC will then be re-promoted from pending to satisfied in the same PR that flips the default. See [ADR-NFM-2739](./ADR-NFM-2739-extraction-job-dual-class.md) for the dataclass contract.
- [ ] **D1 invariant:** The default stated in this ADR for `extraction_v2_enabled` MUST match the default in [`apps/api/src/nfm_db/config.py`](../../apps/api/src/nfm_db/config.py). **Any future flip MUST update both the code and this ADR in the same PR.** If the code default is changed without updating this ADR (or vice-versa), the PR is rejected at review.
...
>
> 2026-08-12 — Correction (supersedes the misleading 2026-08-12 entry that previously appeared here; see [NFM-2935](/NFM/issues/NFM-2935), dispatched from [NFM-2934](/NFM/issues/NFM-2934)). The V2 default flip **has not landed on `main`**. The flip remains on branch `origin/NFM-2876-flip-extraction-v2-default` only ([NFM-2876](/NFM/issues/NFM-2876), PR [#790](https://github.com/Etoile04/nucpot/pull/790), commit `573ddc48`), and NFM-2876 is blocked by [NFM-2909](/NFM/issues/NFM-2909). Per [`apps/api/src/nfm_db/config.py:51`](../../apps/api/src/nfm_db/config.py), the `main` default of `extraction_v2_enabled` is still `False`; V2 is opt-in via `NFM_EXTRACTION_V2_ENABLED=true`. The D1 acceptance criterion above has been reverted to pending/planned phrasing and a same-PR invariant has been added. The prior misleading entry originated from [NFM-2907](/NFM/issues/NFM-2907) / PR #791 (commit `41119444`) and preceded the code change it described; it is hereby corrected and is not an accurate account of the `main` branch state. Dataclass contract still governed by [ADR-NFM-2739](./ADR-NFM-2739-extraction-job-dual-class.md).