...
- [ ] **D1:** Production deploy keeps `NFM_EXTRACTION_V2_ENABLED` unset (or `false`). A canary that flips it ON must fail loudly via `NotImplementedError`, not produce silent zero-result extractions.
