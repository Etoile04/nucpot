...
def upgrade() -> None:
    ...
    # UNITS (21 rows) — split into two statements to keep each <8KB
    # (asyncpg prepare() chokes on large multi-row INSERTs that include
    # unicode characters like °, ·, ², ³, å, μm, Å, etc.).
    bind.execute(
        sa.text(
    ...
    bind.execute(
        sa.text(
            "INSERT INTO unit_conversions (source_unit_id, target_unit_id, factor, offset_value) "
            "SELECT u_from.id, u_to.id, "
            "CASE "
            ...
            "  ELSE 0.0 "
            "END "
            "FROM units u_from JOIN units u_to "
            "ON (u_from.dimension = u_to.dimension AND u_from.symbol != u_to.symbol "
