"""Add dft_calculations table for DFT computation records.

Creates the dft_calculations table storing first-principles DFT calculation
parameters, energy results, structural properties, and metadata.
Links to materials table via nullable FK (material_id).
...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_dft_calcs_material",
        "dft_calculations",
        ["material_id"],
    )
    op.create_index(
        "idx_dft_calcs_calc_id",
        "dft_calculations",
        ["calculation_id"],
    )
    op.create_index(
        "idx_dft_calcs_status",
        "dft_calculations",
        ["status"],
    )
    op.create_index(
        "idx_dft_calcs_functional",
        "dft_calculations",
        ["functional"],
    )
