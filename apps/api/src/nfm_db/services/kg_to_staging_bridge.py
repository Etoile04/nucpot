...
async def bridge_kg_to_staging(
    ...
    for pm in property_measurements:
        ...
        property_name = _PROPERTY_SLUGS.get(
            pm.property_type.name, _slugify(pm.property_type.name)
        )
        if not property_name or property_name == "unknown":
            logger.warning(
                ...
                if text_field:
                    ...
                    if text_row is None:
                        merged_rows[text_key] = {
                            ...
                            "uncertainty": None,
                            "temperature": None,
                            "confidence": confidence_from_property_measurement(
                                pm.review_status
                            ),
                            "context_parts": [f"value_expression={text_field}"],
                            "source_surface": "property_measurements",
                            "kg_node_uuid": None,
                            "property_measurement_uuid": str(pm.id)
                            if pm.id is not None
                            else None,
                        }
                    else:
                        text_row["context_parts"].append(
                            f"value_expression={text_field}"
                        )
            logger.info(
        ...
        if existing_row is not None:
            ...
            if (
                existing_row.get("property_measurement_uuid") is None
                and pm_uuid is not None
            ):
