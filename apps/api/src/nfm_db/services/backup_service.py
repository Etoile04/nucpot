"""Backup scanning and capacity-stats service — NFM-3044.

Provides pure functions for:

- Scanning the backup directory for snapshot files.
- Deriving a GFS retention tier per snapshot based on filename
  convention (``*.hourly.*``, ``*.daily.*``, ``*.weekly.*``).
- Computing real-time disk usage via ``shutil.disk_usage``.
- Reading refusal metrics from a simple JSON counter file.
- Aggregating per-tier ``{count, bytes}`` statistics.

The service is deliberately filesystem-oriented so it works even without a
database — snapshots are files on disk, and refusal state is persisted as
a lightweight JSON sidecar.

Note: tier derivation currently uses the filename suffix convention. When
T1 (NFM-3036 / NFM-3041) ships a content-based ``classifyTier``, swap the
internal helper here without changing the public surface — the snapshot
schema is already tier-aware.
"""
...
def _tier_from_filename(filename: str) -> BackupTier | None:
    ...
    ``BackupTier.HOURLY``. Files without a recognised suffix return
    ``None``.
    """
...
def _load_refusals(path: Path) -> tuple[int, datetime | None]:
    """Read refusal counter and timestamp from a JSON sidecar.

    Returns ``(count, last_refusal_at)``. A missing or corrupt file yields
    ``(0, None)`` rather than raising — operational dashboards must not
    fail when the sidecar is being rotated.
    """
    if not path.exists():
        return 0, None
    try:
...
# ---------------------------------------------------------------------------
# Internal helpers (also reused by tests)
# ---------------------------------------------------------------------------


def _aggregate_tiers(
    snapshots: Iterable[BackupSnapshotResponse],
) -> TierBreakdown:
    """Sum ``count`` and ``bytes`` per tier across *snapshots*."""
    counts: dict[BackupTier, int] = {
        BackupTier.HOURLY: 0,
        BackupTier.DAILY: 0,
        BackupTier.WEEKLY: 0,
    }
    by_bytes: dict[BackupTier, int] = {
        BackupTier.HOURLY: 0,
        BackupTier.DAILY: 0,
        BackupTier.WEEKLY: 0,
    }
    for snap in snapshots:
        counts[snap.tier] += 1
        by_bytes[snap.tier] += snap.size_bytes
    return TierBreakdown(
        hourly=TierStats(count=counts[BackupTier.HOURLY], bytes=by_bytes[BackupTier.HOURLY]),
        daily=TierStats(count=counts[BackupTier.DAILY], bytes=by_bytes[BackupTier.DAILY]),
        weekly=TierStats(count=counts[BackupTier.WEEKLY], bytes=by_bytes[BackupTier.WEEKLY]),
    )
...
def list_snapshots(
    ...
    *,
    ...
    Each file's modification time is used as ``created_at`` (creation time
    is not preserved across scp/ftp in most setups). The tier is derived
    from the filename suffix convention described in
    ...
    *default_tier*.
    ...
    Returns:
        :class:`BackupListResponse` with matching snapshots, sorted newest
        first.
    """
...
def get_backup_stats(
    ...
    *,
    ...
    """Return real-time disk, refusal, and per-tier metrics.

    Uses ``shutil.disk_usage`` on the *backup_dir* volume. Refusal metrics
    are loaded from a JSON sidecar file (if provided and present). The
    per-tier breakdown is computed by scanning the directory the same way
    as :func:`list_snapshots` so the two endpoints stay consistent.

    Args:
        backup_dir: Directory whose volume is measured.
        refusal_file: Path to the refusal JSON sidecar (optional).
        default_tier: Tier assigned to snapshots without a recognised
            filename suffix.
    ...
    try:
        ...
        logger.error("Cannot read disk usage for: %s — falling back to /", dir_path)
    ...
    tiers = _aggregate_tiers(snapshots.snapshots)
