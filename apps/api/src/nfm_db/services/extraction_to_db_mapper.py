...
# ---------------------------------------------------------------------------
# Re-exports (NFM-4620 split — keep the parent's namespace stable)
# ---------------------------------------------------------------------------
# The entity-resolution helpers and the constants they consume moved to
# :mod:`nfm_db.services.extraction_to_db_mapper_lookups` so this module
# could come in under the 1500-line ``File size guard`` ceiling. The
# body of ``map_and_persist`` and the public test API still reference
# them by their original names; re-importing them here keeps both
# surfaces intact without touching the call sites.

from nfm_db.services.extraction_to_db_mapper_lookups import (  # noqa: E402, F401
    # Constants (used by map_and_persist body + NFM-4088 write-path guard tests)
    _BORING_PLACEHOLDER_TITLES,
    _UNATTRIBUTED_SENTINEL_LOCK_KEY,
    _UNATTRIBUTED_SENTINEL_TITLE,
    _UUID_TITLE_PATTERN,
    # Phase + value helpers (called from map_and_persist body)
    _detect_amorphous_phase,
    # Material resolution
    _find_material_by_alias,
    _find_material_by_formula,
    _find_material_by_name,
    _find_material_by_normalized_formula,
    # Source resolution
    _find_source_by_content_md_prefix,
    _find_source_by_doi,
    _find_source_by_title,
    _get_or_create_unattributed_sentinel,
    _has_any_provenance,
    _measurement_value_kwargs,
