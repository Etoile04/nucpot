...
def process_literature_task(self, datasource_id: str) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    ...
    the dispatcher but not yet the service still imports cleanly.  The
    ``_sync`` wrapper bridges Celery's sync worker loop with the async DB
    session used by :func:`process_literature`.
    """
