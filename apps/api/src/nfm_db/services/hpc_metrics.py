...
if PROMETHEUS_AVAILABLE:
    hpc_job_submissions = Counter(
        "hpc_job_submissions_total", "Total HPC job submissions", ["cluster", "status"]
    )
    hpc_job_duration = Histogram("hpc_job_duration_seconds", "HPC job completion time", ["cluster"])
    hpc_file_transfer_bytes = Counter(
        "hpc_file_transfer_bytes_total", "File transfer volume", ["direction", "cluster"]
    )
    hpc_connection_errors = Counter(
        "hpc_connection_errors_total", "SSH connection errors", ["cluster", "error_type"]
    )
    hpc_failover_events = Counter(
        "hpc_failover_events_total", "Failover triggers", ["from_cluster", "to_cluster"]
    )
    hpc_active_connections = Gauge(
        "hpc_active_connections", "Number of active SSH connections", ["cluster"]
    )
