...
References:
    ...
    - NFM-1753: RD-3 audit — cluster-type one-hot features identified as data leakage
    - NFM-1757: PhaseClassifier v2.0 retraining — locked 8D pure physical schema

Schema history:
    - v1.0 (legacy, deprecated): 8D = 4 physical + 4 cluster fractions (cluster_I-IV).
      The cluster fractions are data-leakage sources per NFM-1753.
    - v2.0 (current, locked): 8D pure physical — see ``ML_FEATURE_NAMES`` below.
      Identical to ``PHASE_CLASSIFIER_V2_FEATURE_NAMES`` in ``train_v20.py``.
"""
...
def calculate_mo_equivalent(composition: dict[str, float]) -> float:
    ...
    return sum(
        composition.get(element, 0.0) * coefficient for element, coefficient in lookup.items()
    )
...
def calculate_config_entropy(composition: dict[str, float]) -> float:
    ...
    entropy = -GAS_CONSTANT_R * sum(f * math.log(f) for f in fractions.values() if f > 0)
    return entropy
...
_MIEDEMA_LOOKUP: dict[tuple[str, str], float] = {(a, b): val for a, b, val in _MIEDEMA_PAIRS}
...
def compute_all_features(
    ...
    Example:
        ...
        8
    """
    return {name: calculator(composition) for name, calculator in _FEATURE_CALCULATORS}
...
def calculate_cluster_fractions(composition: dict[str, float]) -> dict[str, float]:
    ...
    .. deprecated::
        Cluster-fraction features are **data-leakage sources** (NFM-1753
        RD-3 audit). The PhaseClassifier v2.0 8D locked schema (NFM-1757)
        no longer includes them. This function is retained **only** for
        legacy v1.0 model loading (``train_v11.py``). Do NOT use for
        new model training or inference.
    ...
    if classified_total > 0:
        return {
            f"cluster_{k}": fractions_by_type[k] / classified_total for k in _CLUSTER_TYPE_LABELS
        }
    return {f"cluster_{k}": 0.0 for k in _CLUSTER_TYPE_LABELS}
...
# Single source of truth: ML_FEATURE_NAMES is locked to the v2.0 schema
# (identical to PHASE_CLASSIFIER_V2_FEATURE_NAMES in train_v20.py per NFM-1757).
# Any change to this list requires a new RD-1 pre-registration.
_PHYSICAL_FEATURE_CALCULATORS_V2: dict[str, Callable] = {
    "mo_equivalent": calculate_mo_equivalent,
    "allen_chi_diff": calculate_allen_chi_diff,
    "config_entropy": calculate_config_entropy,
    "bv_ratio": calculate_bv_ratio,
...
assert set(ML_FEATURE_NAMES) == set(_PHYSICAL_FEATURE_CALCULATORS_V2.keys()), (
    "ML_FEATURE_NAMES must remain in lockstep with _PHYSICAL_FEATURE_CALCULATORS_V2"
)


def compute_ml_features(composition: dict[str, float]) -> dict[str, float]:
    """8D ML feature vector: pure physical schema (PhaseClassifier v2.0 locked).

    Returns the canonical 8D feature vector for ML training and inference.
    Identical schema to ``PHASE_CLASSIFIER_V2_FEATURE_NAMES`` defined in
    ``train_v20.py`` (NFM-1757, locked via RD-1 pre-registration).
    ...
    **Migration note (NFM-1757 / NFM-1829):** The legacy v1.0 schema
    included 4 cluster-fraction features (``cluster_I`` through
    ``cluster_IV``) which are **data-leakage sources** (NFM-1753).
    They have been removed; use ``calculate_cluster_fractions()``
    only for legacy v1.0 model loading.

    Args:
        composition: Element name to atomic percent or fraction mapping.

    Returns:
        Dictionary with 8 keys matching ``ML_FEATURE_NAMES``. All values
        are finite floats. The dictionary iteration order matches
        ``ML_FEATURE_NAMES``.

    Raises:
        ValueError: If composition is empty or all-zero.
    """
    result: dict[str, float] = {}
    for name in ML_FEATURE_NAMES:
        result[name] = _PHYSICAL_FEATURE_CALCULATORS_V2[name](composition)
    return result


def batch_compute_ml_features(compositions: list[dict[str, float]]) -> pd.DataFrame:
    """Batch 8D ML feature vector → DataFrame (v2.0 schema).
