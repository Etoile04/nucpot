...
The schemas below double as FastAPI ``Depends(...)`` request models so
that validators (``le=...``, ``Literal[...]``) run automatically and are
not bypassed by raw ``Query(...)`` bindings on the endpoint.
"""
...
class PropertyQueryRequest(BaseModel):
    ...
    Used both as a Pydantic body-style request model and as an endpoint
    ``Depends(...)`` parameter so validators execute before the service
    is called.
    """

    node_type: str | None = Field(
        None,
        description="Filter by node type (Material, Property, Experiment, Condition, Publication, Measurement)",
    )
...
class RelationQueryRequest(BaseModel):
    ...
    ``direction`` is constrained to the three valid values so callers
    cannot pass arbitrary strings (NFM-858 review M2).
    """
    ...
    relation_type: str | None = Field(
        None,
        description="Filter by relation type (hasProperty, measuredIn, relatedTo, ...)",
    )
...
class PathQueryRequest(BaseModel):
    ...
    ``max_depth`` is hard-capped at ``le=3`` per the spec's p95 <500ms
    target (NFM-858 review M1).
    """
    ...
    max_depth: int = Field(
        ...
        ge=1,
        le=3,
        description="Maximum hop depth (1-3, default 3). Hard cap at 3 per spec.",
    )
    relation_types: list[str] | None = Field(
        None,
        description="Restrict traversal to these relation types (each must be in VALID_RELATION_TYPES)",
    )
    limit: int = Field(
        10,
        ge=1,
