...
async def _seed_pm_environment(db_session, *, source_id, material_formula="U-Mo"):
    ...
    source_id_uuid = (
        _uuid.UUID(source_id) if isinstance(source_id, str) else source_id
    )
    source = DataSource(
        id=source_id_uuid, title="ADR-010 fixture source", source_type="journal"
    )
...
def _pm_stub(
    *,
    ...
    unit_obj = (
        type("UnitNS", (), {"symbol": unit.symbol, "id": unit.id})()
        if unit is not None
        else None
    )
    dataset_obj = (
    ...
    return SimpleNamespace(
        ...
        dataset=dataset_obj,
        property_type_id=(
            property_type.id if property_type is not None else _uuid.uuid4()
        ),
...
async def test_v1_kg_and_pm_collapse_on_5tuple_higher_confidence_wins(
    ...
    collapse_logs = [
        rec for rec in caplog.records if rec.message == "bridge.dedup.collapse"
    ]
    assert len(collapse_logs) == 1, (
        f"expected 1 collapse log entry, got {len(collapse_logs)}"
    )
    rec = collapse_logs[0]
...
@pytest.mark.asyncio
async def test_v2_pm_only_numeric_row_uses_review_status_confidence(
...
@pytest.mark.asyncio
async def test_v3_pm_value_expression_only_no_numeric_row(
    db_session, monkeypatch, caplog
):
    ...
    pm_logs = [
        rec for rec in caplog.records if rec.message == "bridge.pm.expression_only"
    ]
    assert len(pm_logs) == 1, (
        "expected 1 expression-only log entry to carry the text into context"
    )
