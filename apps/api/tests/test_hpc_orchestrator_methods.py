...
class TestSubmitJob:
    ...
    @pytest.mark.unit
    async def test_submit_job_success_on_primary(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch("nfm_db.services.hpc_orchestration.validate_simulation_params"):
            with patch(
                "nfm_db.services.hpc_orchestration.generate_slurm_script",
                ...
                with patch(
                    ...
                    with patch(
                        "nfm_db.services.hpc_orchestration.create_hpc_job_record",
                        new_callable=AsyncMock,
                    ):
    ...
    async def test_submit_job_triggers_failover_when_needed(
        ...
        orchestrator_with_backup.failover_manager.should_trigger_failover = MagicMock(
            return_value=True
        )
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        result = await orchestrator_with_backup.submit_job(
                            "task-1", "structure.xyz", params
                        )
                        assert result == "12345"
    ...
    async def test_submit_job_retries_on_backup_on_submission_error(
        ...
        with patch("nfm_db.services.hpc_orchestration.validate_simulation_params"):
            with patch(
                ...
                with patch(
                    "nfm_db.services.hpc_orchestration.submit_to_slurm", side_effect=mock_submit
                ):
                    with patch(
                        "nfm_db.services.hpc_orchestration.create_hpc_job_record",
                        new_callable=AsyncMock,
        ...
        with patch("nfm_db.services.hpc_orchestration.validate_simulation_params"):
            with patch(
                ...
                with patch(
                    ...
                    with pytest.raises(
                        HPCConnectionError, match="Job submission failed on all clusters"
                    ):
    ...
    @pytest.mark.unit
    async def test_submit_job_creates_hpc_job_record(self, orchestrator: HPCOrchestrator) -> None:
...
class TestPrivateHelpers:
    ...
    def test_generate_slurm_script_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        params = _valid_params()
        with patch(
            "nfm_db.services.hpc_orchestration.generate_slurm_script", return_value="#!/bin/bash"
        ) as mock_gen:
    ...
    async def test_create_hpc_job_record_primary(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration.create_hpc_job_record", new_callable=AsyncMock
        ) as mock_create:
            await orchestrator._create_hpc_job_record("task-1", "12345", _valid_params(), "primary")
            mock_create.assert_called_once_with(
                "task-1", "12345", _valid_params(), "primary.example.com"
            )

    @pytest.mark.unit
    async def test_create_hpc_job_record_backup(
        self, orchestrator_with_backup: HPCOrchestrator
    ) -> None:
        ...
        with patch(
            ...
            await orchestrator_with_backup._create_hpc_job_record(
                "task-1", "12345", _valid_params(), "backup"
            )
            mock_create.assert_called_once_with(
                "task-1", "12345", _valid_params(), "backup.example.com"
            )

    @pytest.mark.unit
    async def test_create_hpc_job_record_backup_no_hosts(
        self, orchestrator: HPCOrchestrator
    ) -> None:
    ...
    def test_parse_walltime_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration.parse_walltime", return_value=120
        ) as mock_parse:
...
class TestPollJobStatus:
    ...
    async def test_poll_returns_running(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch.object(
            orchestrator, "_execute_squeue", new_callable=AsyncMock, return_value="12345 RUNNING"
        ):
    ...
    async def test_poll_returns_pending(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch.object(
            orchestrator, "_execute_squeue", new_callable=AsyncMock, return_value="12345 PENDING"
        ):
            result = await orchestrator.poll_job_status("slurm-12345")
            assert result == "PENDING"

    @pytest.mark.unit
    async def test_poll_returns_completed_when_job_done(
        self, orchestrator: HPCOrchestrator
    ) -> None:
        """Should return COMPLETED when squeue returns None and job is complete."""
        with patch.object(
            orchestrator, "_execute_squeue", new_callable=AsyncMock, return_value=None
        ):
            with patch.object(
                orchestrator, "_check_job_completion", new_callable=AsyncMock, return_value=True
            ):
                result = await orchestrator.poll_job_status("slurm-12345")
                assert result == "COMPLETED"

    @pytest.mark.unit
    async def test_poll_returns_failed_when_job_not_complete(
        self, orchestrator: HPCOrchestrator
    ) -> None:
        ...
        with patch.object(
            ...
            with patch.object(
                orchestrator, "_check_job_completion", new_callable=AsyncMock, return_value=False
            ):
                result = await orchestrator.poll_job_status("slurm-12345")
                assert result == "FAILED"

    @pytest.mark.unit
    async def test_poll_returns_failed_for_unknown_status(
        self, orchestrator: HPCOrchestrator
    ) -> None:
        ...
        with patch.object(
            ...
            "_execute_squeue",
            new_callable=AsyncMock,
            return_value="12345 UNKNOWN_STATE",
        ):
            result = await orchestrator.poll_job_status("slurm-12345")
            assert result == "FAILED"
    ...
    async def test_execute_squeue_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration.execute_squeue",
            new_callable=AsyncMock,
            return_value="output",
        ) as mock_exec:
    ...
    async def test_update_job_status_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration.update_job_status", new_callable=AsyncMock
        ) as mock_update:
    ...
    async def test_check_job_completion_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration.check_job_completion",
            new_callable=AsyncMock,
            return_value=True,
        ) as mock_check:
    ...
    async def test_get_active_jobs_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration.get_active_jobs",
            new_callable=AsyncMock,
...
class TestSyncAllActiveJobs:
    ...
    async def test_syncs_all_active_jobs(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch.object(
            orchestrator, "_get_active_jobs", new_callable=AsyncMock, return_value=[job1, job2]
        ):
            with patch.object(
                orchestrator, "update_job_status", new_callable=AsyncMock
            ) as mock_update:
                await orchestrator.sync_all_active_jobs()
                assert mock_update.call_count == 2

    @pytest.mark.unit
    async def test_sync_continues_on_individual_job_failure(
        self, orchestrator: HPCOrchestrator
    ) -> None:
    ...
    async def test_sync_handles_empty_active_jobs(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch.object(
            orchestrator, "_get_active_jobs", new_callable=AsyncMock, return_value=[]
        ):
            with patch.object(
                orchestrator, "update_job_status", new_callable=AsyncMock
            ) as mock_update:
...
class TestFileTransferDelegates:
    ...
    async def test_upload_file_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._upload_file",
            new_callable=AsyncMock,
            return_value=True,
        ) as mock_upload:
    ...
    async def test_upload_files_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._upload_files",
            new_callable=AsyncMock,
    ...
    async def test_create_task_directory_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._create_task_directory", new_callable=AsyncMock
        ) as mock_create:
    ...
    async def test_download_file_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._download_file",
            new_callable=AsyncMock,
            ...
            mock_dl.assert_called_once_with(
                orchestrator.ssh_manager, "task-1", "/remote/file", "/local/path"
            )
    ...
    async def test_download_results_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._download_results",
            new_callable=AsyncMock,
            return_value=expected,
        ) as mock_dl:
    ...
    async def test_verify_checksum_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._verify_checksum",
            new_callable=AsyncMock,
            return_value=True,
        ) as mock_verify:
    ...
    async def test_get_remote_checksum_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._get_remote_checksum",
            new_callable=AsyncMock,
    ...
    async def test_save_to_object_storage_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._save_to_object_storage",
            new_callable=AsyncMock,
            return_value=expected,
        ) as mock_save:
    ...
    async def test_save_metadata_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._save_metadata", new_callable=AsyncMock
        ) as mock_save:
    ...
    async def test_upload_file_with_retry_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._upload_file_with_retry",
            new_callable=AsyncMock,
            return_value=True,
        ) as mock_upload:
    ...
    async def test_upload_file_with_resume_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._upload_file_with_resume",
            new_callable=AsyncMock,
            return_value=True,
        ) as mock_upload:
    ...
    async def test_get_remote_file_position_delegates(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            "nfm_db.services.hpc_orchestration._get_remote_file_position",
            new_callable=AsyncMock,
            return_value=2048,
        ) as mock_pos:
    ...
    async def test_upload_file_default_max_retries(self, orchestrator: HPCOrchestrator) -> None:
        ...
        with patch(
            ...
            mock_upload.assert_called_once_with(
                orchestrator.ssh_manager, "task-1", "/local", "/remote", 3
            )

    @pytest.mark.unit
    async def test_upload_file_with_resume_default_position(
        self, orchestrator: HPCOrchestrator
    ) -> None:
        ...
        with patch(
            ...
            mock_upload.assert_called_once_with(
                orchestrator.ssh_manager, "task-1", "/local", "/remote", 0
            )
