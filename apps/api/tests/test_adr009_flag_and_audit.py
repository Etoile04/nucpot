"""NFM-3601: ADR-009 §4.3-c — feature flag + audit log writer parity with §4.1.

Verifies:

* Feature flag defaults OFF and reads ``NFM_ADR_009_RECONCILE_ROUTINE_ENABLED``.
...
def flag_module(monkeypatch):
    monkeypatch.delenv("NFM_ADR_009_RECONCILE_ROUTINE_ENABLED", raising=False)
...
class TestAdr009Flag:
    """ADR_009_RECONCILE_ROUTINE_ENABLED env-var gate."""
    ...
    def test_truthy_values_enable(self, flag_module, monkeypatch):
        for truthy in ("true", "1", "yes", "on", "TRUE", " True "):
            monkeypatch.setenv("NFM_ADR_009_RECONCILE_ROUTINE_ENABLED", truthy)
    ...
    def test_falsy_values_stay_off(self, flag_module, monkeypatch):
        for falsy in ("false", "0", "no", "off", "FALSE"):
            monkeypatch.setenv("NFM_ADR_009_RECONCILE_ROUTINE_ENABLED", falsy)
            mod = _reload_flag_module()
            assert mod.is_reconcile_routine_enabled() is False, falsy

    def test_unknown_value_treated_as_off(self, flag_module, monkeypatch):
        monkeypatch.setenv("NFM_ADR_009_RECONCILE_ROUTINE_ENABLED", "banana")
        mod = _reload_flag_module()
...
def audit_module_on(monkeypatch):
    ...
    no-op path.
    """
    monkeypatch.setenv("NFM_ADR_009_RECONCILE_ROUTINE_ENABLED", "true")
    return _reload_audit_module()
...
def _sample_payload(**overrides):
    ...
    payload = {
        ...
        "wake_fired": True,
        "feature_flag": "ADR_009_RECONCILE_ROUTINE_ENABLED",
    }
...
class TestAdr009AuditShape:
    ...
    def test_round_trip_preserves_all_fields(self, audit_session, audit_module_on):
        ...
        assert loaded.feature_flag == "ADR_009_RECONCILE_ROUTINE_ENABLED"
...
class TestAdr009AuditShortCircuit:
    ...
    def test_off_flag_skips_write(self, audit_session, monkeypatch):
        monkeypatch.setenv("NFM_ADR_009_RECONCILE_ROUTINE_ENABLED", "false")
