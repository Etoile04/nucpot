"""Unit tests for MD verification task result-persistence paths.

Split out of ``test_md_tasks.py`` (NFM-1467) to keep that file under the
1500-line CI guard. Covers ``_persist_results`` inner function, session
rollback, outer exception handlers, simulation-params extraction, and
the ``run_md_verification_task`` wrapper.

Usage:
    pytest tests/test_md_tasks_persist.py -v
"""
...
class TestPersistResultsHappyPath:
    ...
    def test_persist_with_full_simulation_data(
        ...
        with (
            ...
            mock_manager_instance.run_verification_pipeline.return_value = verification_results
...
class TestVerificationPipelineErrorHandling:
    ...
    def test_pipeline_generic_exception_becomes_runtime_error(
        ...
        with (
            ...
            mock_manager_instance = Mock()
            mock_manager_instance.run_verification_pipeline.side_effect = RuntimeError(
                "GPU out of memory"
            )
            MockManager.return_value = mock_manager_instance
    ...
    def test_pipeline_keyerror_becomes_runtime_error(
        ...
        with (
            ...
            mock_manager_instance = Mock()
            mock_manager_instance.run_verification_pipeline.side_effect = KeyError("missing_key")
            MockManager.return_value = mock_manager_instance
...
class TestRunMdVerificationTaskWrapper:
    ...
    def test_wrapper_delegates_to_impl(self) -> None:
        ...
        with (
            patch("nfm_db.services.md_tasks._run_md_verification_task_impl") as mock_impl,
        ):
