/**
 * SessionManager — silent token refresh + visible countdown for the frontend.
 *
 * NFM-2236 acceptance criteria:
 ...
 *
 * The module is intentionally framework-agnostic — no React, no DOM, no
 * fetch calls baked in. Wiring is done via the constructor so tests can
 * inject a fake clock and a fake fetch. Production wiring lives in
 * `createBrowserSessionManager()` below.
 */
...
export class SessionManager {
  ...
  refresh(): Promise<void> {
    if (this.isShutdown) return Promise.reject(new Error("manager is shut down"))
  ...
  private transition(next: SessionState): void {
    ...
    for (const sub of this.subscribers) {
      try {
        ...
        // Subscriber errors must not poison other subscribers or the manager.
      }
    }
  }
}
...
/**
 * Default factory wiring SessionManager to the real `fetch` + browser clock.
