 ...
 * internals.
 *
 * NFM-4734 §3 / AC-2: ``reason`` carries the first-class machine-
 * readable code (semantic_timeout / semantic_empty / provider_error /
 * none).  Pre-4734 servers do not emit the field; we default to
 * ``"none"`` so legacy responses stay renderable.  Optional in the
 * contract so legacy test fixtures that build the envelope inline
 * (e.g. ``{ used: false, kind: null, originalError: null }``) keep
 * compiling.
 */
...
export interface RagMessage {
  ...
  /**
   * NFM-4734 §3 / AC-1: the fallback envelope rides on every assistant
   * message so the chat surface (Layout B) renders the same badge that
   * the search page (Layout A) already shows.  Defaults to ``used=false``
   * for messages generated by older code paths that pre-date 4734.
   */
 ...
 *
 * NFM-4734: ``reason`` defaults to ``"none"`` for pre-4734 envelopes
 * (legacy servers do not emit the field).  Used to derive reason from
 * ``kind`` so the chat badge keeps working even on servers that
 * shipped RAG-B but not 4734 — ``kind="iliKE"`` ⇒ ``reason="semantic_timeout"``.
 */
function mapFallback(backend: RagContractFallback | undefined): RagFallbackInfo {
  if (!backend) {
    return { used: false, kind: null, reason: "none", originalError: null }
  }
  const used = Boolean(backend.used)
  let reason: RagFallbackReason = backend.reason ?? "none"
  if (used && reason === "none" && backend.kind) {
    // Legacy envelope shape: kind is set but reason is missing.
    // Map any non-empty kind to semantic_timeout (the only legacy
    // rescue path is the ILIKE text-search fallback).
