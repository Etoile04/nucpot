 ...
 *
 * Adapted from PR #893 (original: `apps/web/src/lib/__tests__/literature-upload-error.test.ts`)
 * to land on the post-NFM-3366 `api-client.ts` (which exposes `ApiError` and
 * `uploadErrorStatus` for all non-OK responses, not just the upload endpoint).
 */
...
describe("literatureApi.upload error status", () => {
  ...
  it('does not classify a 413 as 403 when the byte count contains "403"', async () => {
    ...
    // file. Regression guard for NFM-3359 AC-3.
  ...
  it("preserves the server detail in the user-facing message", async () => {
...
describe("uploadErrorStatus", () => {
  ...
  it("returns the status from a plain object that carries one", () => {
    // Defensive: any thrown value with numeric `status` should be readable.
    expect(uploadErrorStatus({ status: 418 })).toBe(418)
  })
})