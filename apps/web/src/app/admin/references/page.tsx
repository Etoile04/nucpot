...
export default function AdminReferencesPage() {
  ...
  // NFM-4456: replaces the old `submitting` boolean + `setSubmitting` calls
  // scattered around handleSave.
  const saveMutation = useFormSubmit<Record<string, unknown>, void>({
    mutationFn: async () => {
      ...
      if (editingRef) {
        throw new Error("编辑功能暂不可用（后端 API 迁移中）")
      }
  ...
  // NFM-4456: replaces `submittingReview` boolean + try/catch wrapper.
  // Both single and batch review actions route through the same hook so
  // the modal and inline approve/reject buttons share one status.
  const reviewMutation = useFormSubmit<
    { id?: string; ids?: string[]; action: "approve" | "reject"; notes?: string },
    void
  >({
    mutationFn: async (vars) => {
      if (vars.ids) {
        const r = await fetch(`/api/v1/reference-values/bulk`, {
          ...
          body: JSON.stringify({
            action: vars.action,
            staging_ids: vars.ids,
          }),
        })
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
        return
      }
      if (!vars.id) throw new Error("Missing reference id")
      const body: Record<string, unknown> =
        vars.action === "approve" ? { review_note: vars.notes } : { reason: vars.notes }
      const r = await fetch(`/api/v1/reference-values/${vars.id}/${vars.action}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  const fetchReviewItems = useCallback(async () => {
    ...
    try {
      ...
      setReviewItems(
        (data?.data?.records ?? Array.isArray(data))
          ? data
  ...
  const filteredRefs = refs.filter((r) => {
    ...
    if (search) {
      ...
      const searchable =
        `${r.element_system} ${r.property} ${r.source} ${r.source_doi || ""} ${r.method || ""}`.toLowerCase()
  ...
  useEffect(() => {
    setPage(1)
  }, [filterSystem, filterProperty, filterStatus, search])
  ...
  const handleSave = async () => {
    if (!form.element_system || !form.property || !form.value) {
      saveMutation.failWith("请填写必填字段")
      return
    }
    try {
      await saveMutation.submit({})
    } catch {
      // surfaced via saveMutation.error → unified alert
    }
  }
  ...
  const handleSingleReview = async (id: string, action: "approve" | "reject", notes?: string) => {
    try {
      await reviewMutation.submit({ id, action, notes })
    } catch {
      // surfaced via reviewMutation.error
    }
  }
  ...
  const renderError = () => {
    const msg = saveMutation.error ?? reviewMutation.error ?? refsError
    if (!msg) return null
    return (
              ...
              <tbody>
                {pagedRefs.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-gray-300 whitespace-nowrap">
                      {propLabel(r.property)}
                    </td>
                    <td className="px-3 py-2 text-right font-mono text-white whitespace-nowrap">
                      {r.value?.toPrecision(6)}
                    </td>
                    <td className="px-3 py-2 text-gray-400 whitespace-nowrap">{r.unit || "—"}</td>
                    <td className="px-3 py-2 text-right font-mono text-gray-400 whitespace-nowrap">
                      {r.uncertainty != null ? r.uncertainty.toPrecision(4) : "—"}
                    </td>
                    <td
                      className="px-3 py-2 text-gray-400 max-w-[200px] truncate"
                    ...
                    <td
                      ...
                      title={r.source_doi || undefined}
                    >
                      {r.source_doi ? (
                        <a
                          href={`https://doi.org/${r.source_doi}`}
                          target="_blank"
          ...
          <table className="w-full text-sm">
              ...
              <tr className="bg-gray-700">
                <th className="px-3 py-2 text-center text-gray-300 w-10">
                  <input
                    type="checkbox"
                    checked={
                      selectedReviewIds.size === reviewItems.length && reviewItems.length > 0
                    }
                    onChange={() => {
                      if (selectedReviewIds.size === reviewItems.length)
                        setSelectedReviewIds(new Set())
            ...
            <tbody>
              {reviewItems.map((r) => (
                <tr
                  key={r.id}
                  className={`border-b border-gray-700/50 transition ${selectedReviewIds.has(r.id) ? "bg-blue-900/20" : "hover:bg-gray-700/30"}`}
                >
                  ...
                  <td className="px-3 py-2 text-center whitespace-nowrap">
                    <button
                      ...
                      disabled={reviewMutation.isSubmitting}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-gray-800 border border-gray-600 rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl">
            <h2 className="text-lg font-semibold text-white mb-4">
              {reviewAction.type === "approve" ? "批量通过" : "批量拒绝"} ({selectedReviewIds.size}{" "}
              条)
            </h2>
            <textarea
              placeholder={
                reviewAction.type === "approve" ? "审核备注（可选）" : "拒绝原因（可选）"
              }
            ...
            <div className="flex justify-end gap-3 mt-4">
              ...
              >
                {reviewMutation.isSubmitting ? "处理中..." : "确认"}
              </button>
            </div>
          </div>
        </div>
    ...
    if (!matrixData || !matrixData.systems || !matrixData.properties) {
      return (
        <div className="text-gray-500 py-8 text-center">
          ...
          <button onClick={fetchMatrix} className="ml-2 text-blue-400 hover:text-blue-300 text-sm">
            重试
          </button>
        </div>
      )
    ...
    return (
      <div className="bg-gray-800/50 rounded-xl border border-gray-700 overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-gray-700">
              <th className="px-3 py-2 text-left text-gray-300 sticky left-0 bg-gray-700 z-10">
                体系
              </th>
                ...
                >
                  <div className="flex flex-col items-center gap-0.5">
                    ...
                    <span className="text-[10px] text-gray-500">
                      {propCounts[p]?.filled ?? 0}/{propCounts[p]?.total ?? 0}
                    </span>
                  </div>
                </th>
              ))}
          ...
          <tbody>
            {systems.map((s) => {
              let rowFilled = 0
              properties.forEach((p) => {
                if (cells?.[cellKey(s, p)]) rowFilled++
              })

              return (
                <tr key={s} className="border-b border-gray-700/50">
                  <td className="px-3 py-2 font-medium text-white sticky left-0 bg-gray-800 z-10 whitespace-nowrap">
                    {s}
                  </td>
                  <td className="px-3 py-2 text-center text-gray-400 whitespace-nowrap">
                    {rowFilled}/{properties.length} (
                    {((rowFilled / properties.length) * 100).toFixed(0)}%)
                  </td>
                  {properties.map((p) => {
                    ...
                    if (data) {
                      ...
                      const conf =
                        typeof data === "object"
                          ? (data as Record<string, unknown>).confidence
                          : null
                      const colorClass =
                        conf === "A"
                      ...
                      return (
                        <td
                          key={p}
                          className={`px-2 py-2 text-center font-mono whitespace-nowrap ${colorClass}`}
                        >
                    ...
                    return (
                      <td key={p} className="px-2 py-2 text-center text-gray-600">
                        —
                      </td>
                    )
                  })}
          ...
          <div className="bg-gray-800 border border-gray-600 rounded-xl p-6 w-full max-w-lg mx-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                onClick={handleSave}
                disabled={
                  saveMutation.isSubmitting || !form.element_system || !form.property || !form.value
                }
              ...
              >
                {saveMutation.isSubmitting ? "保存中..." : "保存"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-gray-800 border border-gray-600 rounded-xl p-6 w-full max-w-sm mx-4 shadow-2xl">
            ...
            <p className="text-sm text-gray-400 mb-6">
              确定要删除 <span className="text-white">{deleteTarget.element_system}</span> —{" "}
              <span className="text-white">{propLabel(deleteTarget.property)}</span>{" "}
              的参考值吗？此操作不可撤销。
            </p>
