...
import { decodeSlug, findAdjacentPosts, getPublishedPost } from "@/lib/blog/public-posts"
...
export async function generateMetadata({ params }: BlogDetailPageProps): Promise<Metadata> {
  // NFM-4942: params.slug arrives percent-encoded — decode once so the
  // direct FS-seed fallback below can match its decoded index.
...
function makeHeadingComponent(tag: "h1" | "h2" | "h3" | "h4" | "h5" | "h6") {
  return function Heading({ children, ...props }: HeadingProps): ReactElement {
...
export default async function BlogDetailPage({ params }: BlogDetailPageProps) {
  // NFM-4942: decode the percent-encoded param once, up front — the raw
  // form broke both findAdjacentPosts (decoded list → findIndex -1 → dead
  // prev/next nav) and the FS-seed fallback below.
  const { slug: rawSlug } = await params
  const slug = decodeSlug(rawSlug)
  const post = (await getPublishedPost(slug)) ?? getPostBySlug(slug)
