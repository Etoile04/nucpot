...
export default function AdminReferencesPage() {
  ...
  // ─── Tab 3: Matrix state ─────────────────────────────────────────────────
  const [matrixData, setMatrixData] = useState<{ systems: string[]; properties: string[]; cells: Record<string, any> } | null>(null)
  ...
  const handleBatchReview = async () => {
    ...
    try {
      ...
      if (reviewAction.type === 'approve') body.confidence = 'A'
  ...
  const renderMatrixTab = () => {
    ...
    properties.forEach(p => {
      ...
      systems.forEach(s => {
        if (cells?.[cellKey(s, p)]) propCounts[p].filled++
      })
    })
        ...
        <table className="w-full text-xs">
              ...
              {properties.map(p => (
                <th key={p} className="px-2 py-2 text-center text-gray-300 whitespace-nowrap" title={p}>
                  <div className="flex flex-col items-center gap-0.5">
                    ...
                    <span className="text-[10px] text-gray-500">{propCounts[p].filled}/{propCounts[p].total}</span>
                  </div>
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {systems.map(s => {
              let filled = 0
              systems.forEach(_ss => {
                properties.forEach(_pp => { if (cells?.[cellKey(s, _pp)]) filled++ })
              })
              // Count filled for this system row
              let rowFilled = 0
              ...
              return (
                <tr key={s} className="border-b border-gray-700/50">
                  ...
                  {properties.map(p => {
                    ...
                    if (data) {
                      const val = typeof data === 'object' ? (data as any).value : data
                      const conf = typeof data === 'object' ? (data as any).confidence : null
