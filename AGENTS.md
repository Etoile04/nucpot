...
When a PR has approval and is ready to merge, use auto-merge instead of manual merge:

```
gh pr merge <PR_NUMBER> --auto --squash
```

This queues the PR for automatic merge once CI passes. Benefits:
- No need to watch for CI completion
- Reduces CI cascade on remaining PRs
- PR merges automatically when green

Prerequisites: PR must have at least 1 approving review, required status checks configured (done), Allow auto-merge enabled (done).
